...
const sellCandies = (candy) => { 
    return new Promise((resolve, reject) => {
        setTimeout(() => {
        resolve(candy.quantity * 25)}, 3000)
    })
}

const useCandies = async () => {
    const candies = await goGetCandies(); // pause until we get the candy
    const result = await sellCandies(candies) // dependency on previous one
    console.log('async result', result)
}

//console.log(useCandies());

// tricky promise chaining hell 
const candy = goGetCandies();
candy.then((obj) => {
    const sellValue = sellCandies(obj);
    sellValue.then((result) => {
